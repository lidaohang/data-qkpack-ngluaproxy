...
if [ x"$PID" != x ];then
	kill $PID;
	echo "[data-qkpack-ngproxy-qkpack] kill $PID"
fi
...
if [ x"$PID" != x ];then
 	echo "[data-qkpack-ngproxy-qkpack] $BIN -c $CONF $PID restart success."
else
	echo "[data-qkpack-ngproxy-qkpack] $BIN -c $CONF restart failure."
fi