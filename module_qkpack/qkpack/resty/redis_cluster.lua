...
local REDIS_CLUSTER_REQUEST_TTL = 6 --16
...
function _M.get_node_by_slot(self, slot)
    local cluster = clusters[self.cluster_id]
    local node = cluster.slots[slot]
...
function _M.send_cluster_command(self, slotid, cmd, ...)
	...
	ngx.log(ngx.ERR, "redis cluster tries "..tostring(ttl))
	ngx.log(ngx.ERR, "redis cluster err "..err)
